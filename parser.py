...
def convert_excel():
	for file in glob.glob("DataFiles/ASC/*.ASC"):
		...
		with open(file, "r") as asc_file:
			...
			print os.path.abspath(file_pathXL)
		...
		########################################################################################
			...
			Channel_Header = {}
			Channel_Header["Data Source"] = sh.cell_value(8, 2)
			Channel_Header["File Type"] = sh.cell_value(9, 2)
			Channel_Header["Number of Channels"] = sh.cell_value(10, 1)

			dilaContent["Channel Header"] = Channel_Header
		...
		########################################################################################
			...
			channel_1["Channel Type"] = sh.cell_value(11, 1)
			...
			dilaContent["Channel 1"] = channel_1
		...
		########################################################################################
			...
			channel_2["Channel 2"] = [sh.cell_value(17, 1)]
			...
			dilaContent["Channel 2"] = channel_2
		...
		########################################################################################
			...
			channel_3["Channel 3"] = [sh.cell_value(23, 1)]
			...
			dilaContent["Channel 3"] = channel_3
		...
		########################################################################################
			...
			channel_4["Channel 4"] = {}
			channel_4["Channel 4"][""] = sh.cell_value(29, 1)
			channel_4["Channel 4"][""] = sh.cell_value(29, 2)
			...
			dilaContent["Channel 4"] = channel_4
		...
		########################################################################################
			...
			channel_5["Channel 5"] = [sh.cell_value(35, 1)]
			...
			dilaContent["Channel 5"] = channel_5
		...
		#### Not sure what data represents, putting in for use anyway (6/30 @16:26) ####
			...
			dilaContent["Data List"] = {}

			begin = 41 

			while True: 
				try: 
					dilaContent["Data List"]["Column 1"] = sh.cell_value(begin, 0)
					dilaContent["Data List"]["Column 2"] = sh.cell_value(begin, 1)
					dilaContent["Data List"]["Column 3"] = sh.cell_value(begin, 2)

				except: 
					break 

		#### Seemingly repeated data from the initial header. Going to leave out for now (6/30 @16:30) ####

			dilaContent["Data Head Name"] = sh.cell_value(70, 0)

			dilaContent["Data Table"] = {}

			begin = 71 

			while True:
				try: 
					dilaContent["Data Table"]["Column 1"] = sh.cell_value(begin, 1)
					dilaContent["Data Table"]["Column 2"] = sh.cell_value(begin, 2)
					dilaContent["Data Table"]["Column 3"] = sh.cell_value(begin, 3)
					dilaContent["Data Table"]["Column 4"] = sh.cell_value(begin, 4)
					dilaContent["Data Table"]["Column 5"] = sh.cell_value(begin, 5)
			...
			file_pathJSON = 'DataFiles/JSON/%s_1045-R1.json'%sequence
			file_path2 = os.path.abspath(file_pathJSON)
			print file_path2

			file_pathXML = 'DataFiles/XML/%s_1045-R1.xml'%sequence
			file_path3 = os.path.abspath(file_pathXML)
			print file_path3
